...
- **WebRTC 视频流**：连接前可选 VP8/UDP 视频，RFB 继续承载键鼠与剪贴板；传输码率档位和帧率上限同时适用于 VNC 与视频流，视频参数变更会重新协商。需要远端 GStreamer 和 UDP 防火墙规则，失败不会自动回退到 VNC；默认仍使用标准 VNC。
- **断联诊断**：非主动断开时显示脱敏错误报告，可复制后自行提供；不包含凭据、主机地址、剪贴板内容或文件路径，也不会自动发送。
- **文件拖放**：Tauri 连接后可将普通文件拖入窗口，确认后上传到远端 Downloads，再在 QQ 中手动按 `Ctrl+V`（macOS 使用 `Cmd+V`）。
...
1. 在 PenguX11VNC 窗口按 `Ctrl+V`（macOS 使用 `Cmd+V`）；
2. 确认要上传的文件；
3. 切换到 QQ 输入框，再按一次 `Ctrl+V`（macOS 使用 `Cmd+V`）。
...
上传完成后，需要在 QQ 输入框中再次按 `Ctrl+V`（macOS 使用 `Cmd+V`）。应用不会自动粘贴或发送消息。
