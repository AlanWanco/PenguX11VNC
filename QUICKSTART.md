...
在主连接保持运行时，Linux QQ 子窗口关闭后，对应的 Tauri 子窗口也会自动关闭；关闭「自动打开子窗口」不会停止对已打开窗口的回收。本地标题栏关闭使用 Tauri 原生默认行为，远端清理在后台执行，不等待 SSH 才关闭窗口。Tauri 主窗口和 QQ 子窗口会按主窗口的 VNC 缩放倍率统一适配当前屏幕并保持比例，播放区铺满且保留底部状态栏；「系统标题栏」可在设置中开关。关闭后顶部默认显示细深紫色工具条，点击右侧按钮可展开控制按钮。
